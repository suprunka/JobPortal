...
using WcfServiceJobPortal.ServiceInterfaces;
...
namespace WcfServiceJobPortal
{
    ...
    public class OrderService : IOrderService
    {
        ...
        {
            IQueryable<Order> orders = _unitOfWork.Orders.GetAllOrders().Select(x => new Order
            {
                ID = x.ID,
                TotalPrice = x.TotalPrice,
                OrderStatus = x.OrderStatus_ID.ToString(),
                User_ID = x.Users_ID,
                Salelines = x.Salelines.Select(s => new Saleline { Date = s.BookedDate.BookedDate1, ServiceOfferId = s.ServiceOffer_ID, Id = s.ID })
            });
            return orders;
        }
